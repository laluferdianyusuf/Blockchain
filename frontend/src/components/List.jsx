...
export default function List() {
  ...
  const { data: user } = useSWR("user", fetchUserData);

  const { data: certificateData } = useSWR(
    user ? ["certificates", user._id] : null,
    (key, user_id) => dispatch(getCertificatesByUserId(user_id))
  );
  const renderCertificates = () => {
    if (isLoggedIn && certificateData) {
      const certificateArray = certificates?.certificates?.data?.certificates;
